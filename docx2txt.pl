...
Note:	infile.docx can also be a directory name holding the unzipped content
	...
	This Perl script was successfully tested and found to be working
	by using Perl v5.10.1. You can download Perl from http://perl.org if you
	have not installed it yet under your (GNU/)Linux OS via its package
	manager and active Internet connection, or under MS Windows and cmd.exe
	or PowerShell...
	You may have to have a recent version of bash installed. For MS Windows,
	to get bash, download it from cygwin package --- http://cygwin.org .
