...
void initTables(){
  ...
  char *rletters1="абдфгхилмнопруввх";
  char *evowels="aeiouy";
...
void convert(char *buffer, FILE *outstream){
  ...
  while (c=p[shift]){
    if (c1=tab1[c]){shift++;}
      ...
      if (isupper(c)){c1+=32;}
      if (((char)c1=='ь') && (isupper(p[shift-1]))){c1+=32;}
    }
...
uchar s_proc(char* p, int *pshift){
  ...
  /* try to detect "sch" used as "щ" (which is quite common)
   the problem is that
  "сч" combination is quite common too, well, I'm trying 
  to detect most common words, note, "сч" is much less
  frequent than "щ", in piknik "сч" appears 10 times fewer
  than "щ", also, I try to detect if shch already appeared
  ...
  if (LCMP(p[1],'c') && LCMP(p[2],'h')){
    /* frequency of "щ" is 0.3-04% */
    if ((double)shchCnt/(double)letterCnt>0.002) RETURN('с',1);
    /*счастье and its derivatives */
    if (LCMP(p[3],'a')&&LCMP(p[4],'s')&& LCMP(p[5],'t')) RETURN('с',1);
    /* cчитать and its derivatives */
    if (LCMP(p[3],'i')&&LCMP(p[4],'t')&& LCMP(p[5],'a')) RETURN('с',1);
    if (LCMP(p[3],'i')&&LCMP(p[4],'t')&& LCMP(p[5],'a')) RETURN('с',1);
    /*another popular word is счет and its derivatives, but
    how can I distguish it from "ищет","cвищет","щетина","щетка" 
    to make things worse, some might even use "ye" instead of
    "e" , not to mention "писчей бумагой" & "пищей"!! where
    even complete dictionary wouldn't help*/
    RETURN('щ',3);
  }

  /*if the next letter is different from 'h', then it's just с */   
  if (!LCMP(p[1],'h')) RETURN('с',1);
  /* detect shch, which means 'щ', 'шч' pair never appears in Russian*/
  if (LCMP(p[2],'c') && (LCMP(p[3],'h'))) {shchCnt++;RETURN('щ',4);}
  /* now attempt to distinguish 'сх' and 'ш' */
...
uchar y_proc(char *p,int *pshift){
  uchar c1;
  if (LCMP(p[1],'e')){
    /* word cann't end as йе */
    if (islast(p+1)) RETURN('ы',1);
    RETURN('e',2);
  } 
  /* 'ы', I believe, can never be followed by a vowel other than e */
  if (c1=jPair(p[1])) RETURN(c1,2);
  /*no words starting with ы*/    
  if (is1st(p,*pshift)) RETURN('й',1);
  /*ы is never preceded by a vowel, and 'й' is always! */ 
  if (vowel[(uchar)p[-1]]) RETURN('й',1);
  RETURN('ы',1);
}
...
uchar convCombination(char *buffer,int *pshift){
  ...
  case 'c': 
    if (LCMP(p[1],'h'))  RETURN('ч',2);
    RETURN('ц',1);
  case 'k':
    if (LCMP(p[1],'h')) RETURN('х',2);
    RETURN('к',1);
  case 'e':  /* try to distinguish е and э*/
    if (is1st(p,*pshift)){ /*beginning of the word*/
      if (LCMP(p[1],'t') || LCMP(p[1],'k')) RETURN('э',1);
      if (LCMP(p[1],'l') && LCMP(p[2],'e' && LCMP(p[3],'k'))) RETURN('э',1);
    }
    RETURN('е',1);
  case 'z':
    if (LCMP(p[1],'h'))RETURN('ж',2);
    RETURN('з',1);
  case 't':
    if (!LCMP(p[1],'s')) RETURN('т',1);
    if (!LCMP(p[2],'y')) RETURN('т',1); /* ця цю цй невозможны */
    if (!LCMP(p[2],'k')) RETURN('т',1); /* цк  невозможны */
    RETURN('ц',2);
  case '\'':
    /* hard sign never appears in the end of words and before consonants */
    if (islast(p) || (!isvowel(p+1))){RETURN('ь',1);}
    /* 'и' never appears after hard sign (only "юяе" may) */
    if (LCMP(p[1],'i')) RETURN('ь',1);
    RETURN('\'',1);
  }
  RETURN('?',1);
}
